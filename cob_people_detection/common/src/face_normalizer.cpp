...
bool FaceNormalizer::synthFace(cv::Mat &RGB, cv::Mat& XYZ, cv::Size& norm_size, std::vector<cv::Mat>& synth_images)
{
	...
	for (int n = 0; n < synth_images.size(); n++)
	{
		if (config_.cvt2gray)
		{
			if (synth_images[n].channels() == 3)
				cv::cvtColor(synth_images[n], synth_images[n], CV_RGB2GRAY);
		}

		if (config_.eq_ill)
		{
			// radiometric normalization
			if (!normalize_radiometry(synth_images[n]))
				valid = false;
			if (debug_)
				dump_img(synth_images[n], "radiometry");
		}

		if (debug_ && valid)
			dump_img(synth_images[n], "geometry");
...
bool FaceNormalizer::normalizeFace(cv::Mat& img, cv::Mat& depth, cv::Size& norm_size)
{
	...
	if (config_.eq_ill)
	{
		...
		if (!normalize_radiometry(img))
			valid = false;
		if (debug_)
			dump_img(img, "radiometry");
	}

	if (debug_ && valid)
		dump_img(img, "geometry");
...
bool FaceNormalizer::normalize_radiometry(cv::Mat& img)
{
	...
	if (config_.extreme_illumination_condtions == true)
		GammaDoG(img);
	else
		GammaDCT(img);
	return true;
...
bool FaceNormalizer::rotate_head(cv::Mat& img, cv::Mat& depth)
{
	...
	for (int i = 0; i < 60; i++)
	{
		// ----- artificial head pose rotation
		if (i < 20)
			alpha = Eigen::AngleAxis<float>((float)i * 0.1 * M_PI, Eigen::Vector3f(1, 0, 0));
		else if (i >= 20 && i < 40)
			alpha = Eigen::AngleAxis<float>(((float)i - 20) * 0.1 * M_PI, Eigen::Vector3f(0, 1, 0));
		else if (i >= 40 && i <= 60)
			alpha = Eigen::AngleAxis<float>(((float)i - 40) * 0.1 * M_PI, Eigen::Vector3f(0, 0, 1));
