...
FaceNormalizer::FaceNormalizer(int i_epoch_ctr, bool i_debug, bool i_record_scene, std::string i_dbg_path) :
	epoch_ctr(i_epoch_ctr), debug_(i_debug), record_scene(i_debug),
	//HOME
			//debug_path_("/home/tom/git/care-o-bot/cob_people_perception/cob_people_detection/debug/"),
			//IPA
			debug_path_(i_dbg_path), kinect(VirtualCamera::KINECT), vis_debug_(false)
{
	this->init();
}
FaceNormalizer::FaceNormalizer(FNConfig& config) :
	epoch_ctr(0), debug_(false), record_scene(false),
	//HOME
			//debug_path_("/home/tom/git/care-o-bot/cob_people_perception/cob_people_detection/debug/"),
			//IPA
			debug_path_("/share/goa-tz/people_detection/normalization/results/"), kinect(VirtualCamera::KINECT), vis_debug_(false), config_(config)
{
	this->init();
}
FaceNormalizer::FaceNormalizer() :
	epoch_ctr(0), debug_(true), record_scene(true),
	//HOME
			//debug_path_("/home/tom/git/care-o-bot/cob_people_perception/cob_people_detection/debug/"),
			//IPA
			debug_path_("/share/goa-tz/people_detection/normalization/results/"), kinect(VirtualCamera::KINECT), vis_debug_(true)
{
	config_.eq_ill = true;
	config_.align = true;
	config_.resize = true;
	...
	if (config_.align)
	{
		...
		if (config_.eq_ill)
		{
			...
			if (!normalize_radiometry(img))
				valid = false;
			if (debug_)
				dump_img(img, "1_radiometry");
		}
		if (debug_ && valid)
			dump_img(img, "1_geometry");
	}
...
bool FaceNormalizer::normalizeFace(cv::Mat& img, cv::Size& norm_size)
{
	...
	//geometric normalization
	{
		if (!normalize_geometry(img, FaceNormalizer::AFFINE))
			valid = false;
		if (debug_)
			dump_img(img, "1_geometryRGB");
	}
...
bool FaceNormalizer::normalize_geometry_depth(cv::Mat& img, cv::Mat& depth)
{
	...
	eye_middle = lefteye + ((x_new.dot(nose - lefteye)) / (x_new.dot(x_new))) * x_new;
	...
	cv::Mat imgres;
	if (img.channels() == 3)
		imgres = cv::Mat::zeros(480, 640, CV_8UC3);
	if (img.channels() == 1)
		imgres = cv::Mat::zeros(480, 640, CV_8UC1);
	...
	//TODO
	if (roi.height <= 0 || roi.width <= 0 || roi.x < 0 || roi.y < 0 || roi.x + roi.width > imgres.cols || roi.y + roi.height > imgres.rows)
		return false;
